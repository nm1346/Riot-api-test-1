	...
	public Object process(String username) {
		...
		try {
			...
			for (int i = 0; i < gameInfo.getParticipants().size(); i++) {
				...
				//
				// }
				int spell = (int) gameInfo.getParticipants().get(i).getSpell1Id();
				int spel2 = (int) gameInfo.getParticipants().get(i).getSpell2Id();

				if (i <= 4) {
					list1.add(summonerDao.selectchampionKey(chamid));
					spell_list1.add(riotApiManager.getSummonerSpell(spell));
					spell_list2.add(riotApiManager.getSummonerSpell(spel2));
					...
					spell_list3.add(riotApiManager.getSummonerSpell(spell));
					spell_list4.add(riotApiManager.getSummonerSpell(spel2));
